...
impl<'a> Serialize for RunStep<'a> {
	fn serialize<S: Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
		...
		m.serialize_entry("remaining_script_hex", &FmtSer(&self.remaining_script.as_bytes().as_hex()))?;
		m.serialize_entry("remaining_script_asm", &FmtSer(&ScriptAsm(self.remaining_script)))?;
